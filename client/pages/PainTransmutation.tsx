...
function TransmutationStudio() {
  ...
  const [activeResult, setActiveResult] = useState<TransmutationResult | null>(
    null,
  );
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-foreground/70 max-w-2xl mx-auto">
          Transform crisis into healing through AI-powered alchemy. Submit your
          challenges and receive poetry, rituals, music, and narratives for
                  ...
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 text-left transition-all ${
                      selectedType === type.id
                        ? type.color === "atlas-gold"
                          ? "border-atlas-gold bg-atlas-gold/5"
                          : type.color === "atlas-cosmic"
                            ? "border-atlas-cosmic bg-atlas-cosmic/5"
                            : type.color === "atlas-regenerative"
                              ? "border-atlas-regenerative bg-atlas-regenerative/5"
            ...
            {results.length === 0 ? (
              <div className="text-center py-12 text-foreground/60">
                ...
                <p className="text-sm mt-2">
                  Share your crisis to begin the healing process
                </p>
              </div>
            ) : (
        ...
        <Card className="border-atlas-gold/50">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                {transmutationTypes.find((t) => t.id === activeResult.type)
                  ?.icon && (
                  <div className="w-6 h-6">
                    {(() => {
                      const IconComponent = transmutationTypes.find(
                        (t) => t.id === activeResult.type,
                      )?.icon;
                      return IconComponent ? (
                        <IconComponent className="w-full h-full text-atlas-gold" />
                      ) : null;
                    })()}
                  </div>
                )}
...
export default function PainTransmutation() {
  ...
  const structuredData = {
    ...
    description:
      "AI-powered platform for transforming personal and collective crises into healing poetry, sacred rituals, and regenerative music.",
    ...
    audience: {
      "@type": "Audience",
      audienceType:
        "Artists, Healers, Spiritual Practitioners, Community Workers",
    },
  };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="border-b border-border bg-background/80 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link
            to="/"
            className="flex items-center space-x-2 text-foreground/80 hover:text-atlas-gold transition-colors"
          >
          ...
          <div className="flex items-center space-x-4">
            {state.user.persona && (
              <Badge variant="secondary">
                {getPersonaConfig(state.user.persona)?.icon}{" "}
                {getPersonaConfig(state.user.persona)?.title}
              </Badge>
            )}
          </div>
        </div>
