...
const sampleDispatch: DispatchData = {
  ...
  date: "December 2024",
  invocation:
    "This month, $1.3B moved from harm to healing. Two new regenerative covenants have been born. And we tell the story of a tree planted in Mali that began as a financial number.",
  mapSnapshot: {
    ...
    regionSpotlight: {
      ...
      story:
        "A reforestation covenant connected Swedish pension funds with Malian communities, transforming financial abstractions into living roots.",
  ...
  custodianStory: {
    ...
    transformation:
      "Redirected $50M from extractive mining to regenerative agriculture, creating a model for institutional investors seeking meaning beyond returns.",
    quote:
      "I realized my spreadsheets were actually prayers for the planet. Every cell could hold healing instead of harm.",
    avatar: "💫",
  },
  wisdomArchive: {
    quote:
    ...
    connection:
      "This wisdom guides our covenant capital principles - every investment is a relationship, not a transaction.",
  },
  actions: [
    {
...
export default function SanctumDispatch({
  data = sampleDispatch,
  isPreview = false,
}: SanctumDispatchProps) {
  return (
      ...
      <div className="space-y-8">
              ...
              <div className="flex-1">
                <div className="mb-2">
                  <h3 className="font-semibold text-foreground">
                    {data.custodianStory.name}
                  </h3>
                  <p className="text-sm text-foreground/70">
                    {data.custodianStory.role}
                  </p>
                </div>
        ...
        <Card className="border-atlas-cosmic/30">
          ...
          <CardContent>
            ...
            <div className="text-right mb-4">
              <span className="text-atlas-cosmic font-medium">
                — {data.wisdomArchive.author}
              </span>
            </div>
            <div className="bg-muted/30 p-4 rounded-lg">
              <p className="text-sm text-foreground/70 leading-relaxed">
                <strong>Connection to Finance:</strong>{" "}
                {data.wisdomArchive.connection}
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-4">
              {data.actions.map((action, index) => (
                ...
                >
                  ...
                  >
                    {action.type === "explore" ? (
                      <Globe className="w-5 h-5 text-atlas-wisdom" />
                    ) : action.type === "join" ? (
                      <Users className="w-5 h-5 text-atlas-cosmic" />
                    ) : (
        ...
        {isPreview && (
          <div className="bg-atlas-cosmic/10 border border-atlas-cosmic/30 rounded-lg p-4 text-center">
            <p className="text-atlas-cosmic font-medium mb-2">
              This is a preview of The Sanctum Dispatch
            </p>
            <p className="text-sm text-foreground/70">
              Subscribe to receive monthly wisdom, impact stories, and
              regenerative actions in your inbox
            </p>
          </div>
        )}
      </div>
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-foreground/70">
          A monthly letter where capital stories become healing wisdom
        </p>
      </div>
      ...
      {onSubscribe && (
        <div className="text-center pt-6">
          <Button
            onClick={onSubscribe}
            className="bg-atlas-cosmic hover:bg-atlas-cosmic/90 text-white"
          >
