...
self.addEventListener('fetch', (event) => {
  ...
  if (request.destination === 'document') {
    event.respondWith(
      staleWhileRevalidate(request, DYNAMIC_CACHE_NAME)
    );
    return;
  }
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: event.data ? event.data.text() : 'New content available in Atlas Sanctum',
  ...
  event.waitUntil(
    self.registration.showNotification('Atlas Sanctum', options)
  );
});
